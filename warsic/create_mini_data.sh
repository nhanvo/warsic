#!/bin/sh 
#-xv
echo "Creating mini data..."
PLATFORM=mini
OUTPUT_FOLDER=data_mini
...
# Copy mini specific data
#echo "Copy data_org_mini ..."
#cp -rfp data_org_mini/. $OUTPUT_FOLDER/

# Copy music files into data folder
#mkdir $OUTPUT_FOLDER/music
#cp -rfp music_org/mini/*.mp3 $OUTPUT_FOLDER/music/
...
cp services.desc android/assets/

cd $OUTPUT_FOLDER
#rm *.djz
#zip arch0.djz -rq animations/ effects/ fonts/ materials/ shaders/ text/ ui/ sound/ textures/ backgrounds/ sprites/ objects/ enemies/ enemies_phone/ levels/ levels_phone/ story/
#zip arch0.djz -rq0 music/
#rm -r $FULL_FOLDER_PACKAGES
#mkdir $FULL_FOLDER_PACKAGES
#mv *.djz $FULL_FOLDER_PACKAGES
#cp -rfp ../music_org/mini/ $FULL_FOLDER_PACKAGES/music/
